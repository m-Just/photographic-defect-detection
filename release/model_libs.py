...
def add_act_layer(net, from_layer, layer_name, fn, in_place):

  if fn == 'relu':
    net[layer_name] = L.ReLU(net[from_layer], in_place=False)
  elif fn == 'leaky_relu':
    net[layer_name] = L.ReLU(net[from_layer], in_place=False, negative_slope=0.2)
  elif fn == 'sigmoid':
    net[layer_name] = L.Sigmoid(net[from_layer], in_place=False)
  elif fn == 'tanh':
    net[layer_name] = L.TanH(net[from_layer], in_place=False)
# def input_layer(net, shape_, data_name):
#     kwargs = {
#     'input_dim': shape_[0],
#     'input_dim': shape_[1],
#     'input_dim': shape_[2],
#     'input_dim': shape_[3]
#     }
#     return L.Net(name=data_name,  input_dim=shape_)


def ConvBNLayer(net, from_layer, out_layer, use_bn, act_f, conv_f, num_output,
    kernel_size, pad, stride, dilation=1, use_scale=True, lr_mult=1,
    group=1, interP_height=0, interP_width=0,
  ...
  if use_bn:
    ...
    # eps = bn_params.get('eps', 0.00001)
    # moving_average_fraction = bn_params.get('moving_average_fraction', 0. )
  ...
  if conv_f == 'conv':
    ...
    if kernel_h == kernel_w:
      kwargs['convolution_param'] = dict(num_output=num_output, kernel_size=kernel_h,pad=pad_h,stride=stride_h)
      net[conv_name] = L.Deconvolution(net[from_layer], **kwargs)
    else:
      kwargs['convolution_param'] = dict(num_output=num_output, kernel_w=kernel_w,kernel_h=kernel_h,pad_h=pad_h,pad_w=pad_w,stride_h=stride_h,stride_w=stride_w)
      net[conv_name] = L.Deconvolution(net[from_layer],**kwargs)
  elif conv_f == 'bilinear_deconv':
    if kernel_h == kernel_w:
        # dconvwargs['convolution_param'] =dict(weight_filler=dict(type='bilinear'),bias_term=False,
        #     num_output=2,
        #     kernel_size=3,
        #     stride=2,
        #     pad=1,
        #     group=2)
        up_name = '{}_up_bilinear'.format(conv_name)
        net[up_name] = L.Interp(net[from_layer], height=interP_height, width=interP_width)
        convargs = {}
        convargs['convolution_param'] = dict(num_output=num_output, kernel_size=kernel_h,pad=pad_h,stride=stride_h)
        net[conv_name] = L.Convolution(net[up_name], **convargs)
    else:
        up_name = '{}_up_bilinear'.format(conv_name)
        net[up_name] = L.Interp(net[from_layer], height=interP_height, width=interP_width)
        convargs = {}
  ...
  if use_bn:
    ...
    net[bn_name] = L.BatchNorm(net[conv_name], in_place=False, **bn_kwargs)
    cur_layer = bn_name
    if use_scale:
      sb_name = '{}{}{}'.format(scale_prefix, out_layer, scale_postfix)
      net[sb_name] = L.Scale(net[bn_name], in_place=False, **sb_kwargs)
      cur_layer = sb_name
    # else:
    #   bias_name = '{}{}{}'.format(bias_prefix, out_layer, bias_postfix)
    #   net[bias_name] = L.Bias(net[bn_name], in_place=False, **bias_kwargs)
    #   cur_layer = bias_name
  if act_f == 'relu':
    relu_name = '{}_{}'.format(conv_name, act_f)
    net[relu_name] = L.ReLU(net[cur_layer], in_place=False)
    cur_layer = relu_name
  elif act_f == 'leaky_relu':
    leaky_name = '{}_{}'.format(conv_name, act_f)
    net[leaky_name] = L.ReLU(net[cur_layer], in_place=False, negative_slope=0.2)
    cur_layer = leaky_name
  elif act_f == 'sigmoid':
    sgd_name = '{}_{}'.format(conv_name, act_f)
    net[sgd_name] = L.Sigmoid(net[cur_layer], in_place=False)
    cur_layer = sgd_name
  elif act_f == 'tanh':
    th_name = '{}_{}'.format(conv_name, act_f)
    net[th_name] = L.TanH(net[cur_layer], in_place=False)
    cur_layer = th_name
  return cur_layer
